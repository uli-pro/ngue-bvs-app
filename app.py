...
def checkout_erfolg():
    ...
    # Session-Reservierungen aufräumen (deterministic cleanup)
    try:
        cleared = VerseReservation.clear_for_session(session.sid)
        if cleared > 0:
            app.logger.info(f"Cleared {cleared} session reservations for session {session.sid[:8]}...")
    except Exception as e:
        app.logger.warning(f"Session reservation cleanup failed: {e}")

    # Background cleanup: expired reservations + orphaned pending donations
    try:
        ...
        if expired_res > 0 or orphaned_don > 0:
            app.logger.info(f"Background cleanup: {expired_res} expired reservations, {orphaned_don} orphaned donations")
    except Exception as e:
        app.logger.warning(f"Background cleanup failed: {e}")
