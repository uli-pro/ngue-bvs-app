#!/usr/bin/env python3
"""
Keyword-basierter Test für die Bibelvers-Suche
...
class KeywordSearchTester:
    def __init__(self, dataset_file='verses_subset_1000.json'):
        """Initialisiere Tester"""
        self.table_name = 'keyword_test_verses'
        ...
        # Lade Dataset
        print(f"\nLade Dataset aus {dataset_file}...")
        try:
            with open(dataset_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                self.verses = data['verses']
                self.verse_count = len(self.verses)
                print(f"Geladen: {self.verse_count} Verse")
    ...
    def setup_database(self):
        ...
        # Füge Verse ein
        print("Füge Verse in Datenbank ein...")
        verses_data = [
            (
                verse['reference'],
                verse['book'],
        ...
        print(f"✓ Tabelle '{self.table_name}' erstellt und gefüllt")
    ...
    def run_all_tests(self):
        ...
        for i, keyword in enumerate(TOP_50_KEYWORDS, 1):
            ...
            print(f"\n{i}. '{keyword}' - {result['count']} Treffer ({result['search_time']:.3f}s)")
    ...
    def save_results(self, results):
        ...
        filename = f"results/keyword_test_results_{timestamp}.json"
    ...
    def create_markdown_report(self, results, timestamp):
        ...
        filename = f"results/keyword_test_report_{timestamp}.md"
        
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("# Keyword Search Test Report\n\n")
            f.write(f"**Datum**: {results['metadata']['test_date']}\n")
            f.write(f"**Verse in Datenbank**: {results['metadata']['verse_count']}\n\n")
            
            # Statistiken
            total_single = sum(r['count'] for r in results['single_keywords'])
            ...
            f.write(f"- **Keyword-Paare**: {len(results['keyword_pairs'])} getestet, {total_pairs} Treffer insgesamt\n")
            f.write(f"- **Keyword-Tripel**: {len(results['keyword_triples'])} getestet, {total_triples} Treffer insgesamt\n\n")
            
            # Top Keywords nach Treffern
            f.write("## Top 10 Keywords nach Anzahl Treffer\n\n")
            sorted_keywords = sorted(results['single_keywords'], key=lambda x: x['count'], reverse=True)[:10]
            for i, kw in enumerate(sorted_keywords, 1):
                f.write(f"{i}. **{kw['keyword']}**: {kw['count']} Treffer\n")
            
            # Keywords ohne Treffer
            no_results = [kw['keyword'] for kw in results['single_keywords'] if kw['count'] == 0]
            if no_results:
            ...
            for kw in results['single_keywords']:
                f.write(f"#### {kw['keyword']} ({kw['count']} Treffer, {kw['search_time']:.3f}s)\n\n")
                if kw['count'] > 0:
...
def main():
    ...
    print("Testet die 50 häufigsten Keywords + Kombinationen")
